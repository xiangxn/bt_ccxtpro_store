...
class MetaCCXTProBroker(BrokerBase.__class__):
    ...
    def __init__(cls, name, bases, dct):
        ...
        super(MetaCCXTProBroker, cls).__init__(name, bases, dct)
        CCXTProStore.BrokerCls = cls


class CCXTProBroker(with_metaclass(MetaCCXTProBroker, BrokerBase)):
    ...
    def __init__(self, broker_mapping=None, debug=False, **kwargs):
        super(CCXTProBroker, self).__init__()
        ...
        self.store = CCXTProStore(**kwargs)
    ...
    def start(self):
        super(CCXTProBroker, self).start()

        if self.p.use_positions:
            pass  # not yet implemented

    async def get_balance(self, data=None):
        if data is None:
            await self.store.get_balance()
        else:
            vn = data.p.dataname.replace(self.currency, "").replace("/", "")
            await self.store.get_balance(vn)
    ...
    async def next(self):
        ...
        for o_order in list(self.open_orders):
            ...
            # Get the order
            ccxt_order = await self.store.fetch_order(oID, o_order.data.p.dataname)
    ...
    async def _submit(self, owner, data, exectype, side, amount, price, params):
        ...
        if not self.use_order_params:
            ret_ord = await self.store.create_order(symbol=data.p.dataname, order_type=order_type, side=side, amount=amount, price=price, params={})
        else:
            try:
                ...
                ret_ord = await self.store.create_order(symbol=data.p.dataname, order_type=order_type, side=side, amount=amount, price=price, params=params)
            except:
    ...
    async def buy(self,
        ...
        del kwargs['parent']
        del kwargs['transmit']
        return await self._submit(owner, data, exectype, 'buy', size, price, kwargs)
    ...
    async def sell(self,
        ...
        del kwargs['parent']
        del kwargs['transmit']
        return await self._submit(owner, data, exectype, 'sell', size, price, kwargs)
    ...
    async def cancel(self, order):
        ...
        ccxt_order = await self.store.fetch_order(oID, order.data.p.dataname)
        ...
        ccxt_order = await self.store.cancel_order(oID, order.data.p.dataname)
    ...
    async def get_orders_open(self, safe=False):
        return await self.store.fetch_open_orders()

    async def private_end_point(self, type, endpoint, params, prefix=""):
        '''
        ...
        return await self.store.private_end_point(type=type, endpoint=method_str, params=params)
