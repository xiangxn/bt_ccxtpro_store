...
class MetaCCXTFeed(DataBase.__class__):
    ...
    def __init__(cls, name, bases, dct):
        ...
        CCXTProStore.DataCls = cls


class CCXTProFeed(with_metaclass(MetaCCXTFeed, DataBase)):
    """
    ...
    async def _fetch_ohlcv(self, fromdate=None):
            ...
            if self.p.debug:
                ...
                try:
                    for i, ohlcv in enumerate(data):
                        ...
                        print('{} - Data {}: {} - TS {} Time {}'.format(datetime.utcnow(), i, datetime.utcfromtimestamp(tstamp // 1000), tstamp,
                                                                        (time.time() * 1000)))
                        # ------------------------------------------------------------------
                except IndexError:
    ...
    async def _load_ticks(self):
        if self._last_id is None:
            # first time get the latest trade only
            trades = [await self.store.fetch_trades(self.p.dataname)[-1]]
        else:
            trades = await self.store.fetch_trades(self.p.dataname)
